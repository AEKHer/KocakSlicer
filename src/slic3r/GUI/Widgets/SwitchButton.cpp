...
SwitchButton::SwitchButton(wxWindow* parent, wxWindowID id)
    ...
    , thumb_color(std::pair{0xDC2626, (int) StateColor::Checked}, std::pair{0xD9D9D9, (int) StateColor::Normal})
{
...
void SwitchBoard::doRender(wxDC &dc)
{
	...
	/*left*/
    if (switch_left) {
        is_enable ? dc.SetBrush(wxBrush(wxColour(220, 38, 38))) : dc.SetBrush(disable_color);
	...
	/*right*/
    if (switch_right) {
        if (is_enable) {dc.SetBrush(wxBrush(wxColour(220, 38, 38)));
        } else {dc.SetBrush(disable_color);}
